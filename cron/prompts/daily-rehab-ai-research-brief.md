...
운영 전달 정책:
- 검증 통과 신규 재활 AI 논문/뉴스가 0건이면 terminal 작업, Notion 라우터 판단, 후보 파일, manifest 기록까지 완료한 뒤 최종 응답은 정확히 `[SILENT]` 하나로만 끝낸다.
- 이 0건 무소식 케이스에서는 위 최종 출력 형식과 필수 섹션 요구를 모두 무시한다. 설명, 요약, 오류 보고, 섹션 제목, 결론을 절대 쓰지 말고 `[SILENT]` 외의 어떤 문자도 출력하지 않는다.
- Discord에 "신규 없음" 상태 보고를 보내지 않는다.
- 검증 통과 항목이 1건 이상일 때만 정상 브리핑 형식으로 전달한다.
