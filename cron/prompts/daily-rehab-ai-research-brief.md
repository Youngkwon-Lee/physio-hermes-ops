...
- `date`는 공식 논문·기관 원문의 게시일인 YYYY-MM-DD 값이다. 원문을 직접 읽은 뒤에만 `source_url_verified: true`로 기록한다.
- `source_claims`에는 원문에서 확인한 짧은 사실 문장 1~3개를 넣고, Discord 본문과 내부 JSON 필드를 섞지 않는다.
