...
- [ ] `ORCH_STATUS_V2.md`의 RED/YELLOW 상태와 충돌 없이, “실제 최종 카피 부재”가 리스크로 반영되어 있다.
...
- [ ] 카드 클래스에 기본 시각 스타일이 포함되어 있다.
  ...
  - [ ] `shadow-sm`
- [ ] 현재 목업이 단일 카드만 제공한다는 한계가 문서에 반영되어 있다.
- [ ] “같은 행 카드 간 간격 일관성”은 단일 카드만으로 완전 검증 불가하다고 명시한다.
...
- 로그인 오류 문구의 실제 최종 카피가 sandbox에 없어 작업 1은 완료 판정 근거가 부족할 수 있다.
- KPI 카드는 단일 목업만 있어 작업 2의 “같은 행 간격 일관성”은 부분 검증에 그칠 수 있다.
