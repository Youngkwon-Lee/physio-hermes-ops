...
- 경계:
  - `automation_control_center_read_model.json`은 Kinelo Ops의 `kinelo-automation-control-center-materialize.service`가 단독 생성한다.
  - `runtime-health-snapshot.service`는 위 runtime health 산출물만 갱신하고 Control Center read model을 다시 만들지 않는다.
